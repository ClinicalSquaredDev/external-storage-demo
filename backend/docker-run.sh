...
if [ "${IS_BOOTNODE}" = "false" ]; then
  if [ ! -f "${CONFIG_DIR_PATH}/${DEPLOY_FILE_NAME}" ]; then
    echo "IS_BOOTNODE=false but there was no ${DEPLOY_FILE_NAME} provided in external-storage-demo_config docker volume. Exit."
    exit 53
  fi
else
...
if [ ! -f "${CONFIG_DIR_PATH}/config.yaml" ]; then
  ...
  # Generating the ./config/generated.config.yaml - an intermediate step to avoid removing SERVER var (that would break the non-docker deployment)
...
echo 'Starting external-storage-demo-server...'
yarn start:prod
