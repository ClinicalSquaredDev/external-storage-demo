...
docker stop external-storage-demo_http-redirect_1
...
function cleanup {
  docker-compose down
  docker start external-storage-demo_http-redirect_1
}
...
if [[ "${DRY_RUN}" = "true" ]]; then
  ...
  echo "Copying key and cert to the external-storage-demo nginx ssl dir..."
  set -x
  ...
  set -x
  sudo docker cp --follow-link ${cert_path} external-storage-demo_nginx_1:/etc/ssl/server.pem
  sudo docker cp --follow-link ${key_path} external-storage-demo_nginx_1:/etc/ssl/server.key
  ...
  set -x
  sudo docker exec external-storage-demo_nginx_1 nginx -s reload
